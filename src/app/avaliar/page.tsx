...
import { Search, Star, ChevronRight, ChevronLeft, CheckCircle, AlertCircle, Upload } from 'lucide-react';
...
export default function AvaliarPage() {
  ...
  const [data, setData] = useState({
    schoolName: '', city: '', unit: '', stage: '', year: '',
      ...
      <>
        ...
        <div className="pt-16 min-h-screen bg-cream flex items-center justify-center">
          <div className="card text-center max-w-md mx-4">
            ...
            <p className="text-gray-500 mb-6">Obrigado por contribuir com a comunidade Avali. Sua avaliação passa por moderação e será publicada em breve.</p>
            <div className="flex gap-3 justify-center">
              <a href="/buscar" className="btn-primary text-sm">Ver outras escolas</a>
              <a href="/" className="btn-secondary text-sm">Início</a>
            </div>
          </div>
        </div>
        <Footer />
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <input value={data.schoolName} onChange={e => setField('schoolName', e.target.value)}
                      placeholder="Nome da escola..." className="input pl-10" />
                  </div>
                  <input value={data.city} onChange={e => setField('city', e.target.value)}
                    placeholder="Cidade" className="input" />
                  <input value={data.unit} onChange={e => setField('unit', e.target.value)}
                    placeholder="Unidade (se houver)" className="input" />
                  <div className="grid grid-cols-2 gap-3">
                    <select value={data.stage} onChange={e => setField('stage', e.target.value)} className="input">
                      <option value="">Etapa do aluno</option>
                      {STAGES.map(s => <option key={s}>{s}</option>)}
                    </select>
                    <select value={data.year} onChange={e => setField('year', e.target.value)} className="input">
            ...
            {step === 2 && (
              <div>
                ...
                <div className="mb-6 p-4 bg-cream-card rounded-2xl">
                  ...
                  <StarInput value={data.overallRating} onChange={v => setField('overallRating', v)} />
                </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea value={data.comment} onChange={e => setField('comment', e.target.value)}
                      rows={4} placeholder="Descreva sua experiência com a escola..."
                      className="input resize-none" />
                  </div>
                  <div>
                    <label className="text-sm font-bold text-green-700 block mb-2">✓ Pontos positivos</label>
                    <textarea value={data.pros} onChange={e => setField('pros', e.target.value)}
                  ...
                  <div>
                    ...
                    <textarea value={data.cons} onChange={e => setField('cons', e.target.value)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setField('wouldRecommend', true)}
                    ...
                    <button onClick={() => setField('wouldRecommend', false)}
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input value={data.monthlyFee} onChange={e => setField('monthlyFee', e.target.value)}
                    ...
                    <div>
                      ...
                      <input value={data.enrollment} onChange={e => setField('enrollment', e.target.value)}
                        placeholder="R$ 0,00" className="input" type="number" min="0" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input value={data.meals} onChange={e => setField('meals', e.target.value)}
                        placeholder="R$ 0,00" className="input" type="number" min="0" />
                    </div>
                  </div>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2">
                    ...
                    <p className="text-xs text-amber-800">Não envie documentos com dados pessoais de crianças ou responsáveis. A Avali remove automaticamente informações sensíveis antes de qualquer publicação.</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-4">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" checked={data.isAnonymous} onChange={e => setField('isAnonymous', e.target.checked)}
                      className="accent-coral mt-0.5 w-4 h-4" />
                    <div>
                      <p className="font-semibold text-navy text-sm">Publicar como anônimo</p>
                      <p className="text-xs text-gray-500">Seu nome não aparecerá na avaliação. Dados de auditoria são mantidos internamente para segurança e moderação, conforme nossa Política de Privacidade.</p>
                    </div>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" checked={data.acceptTerms} onChange={e => setField('acceptTerms', e.target.checked)}
                      className="accent-coral mt-0.5 w-4 h-4" />
                    <p className="text-xs text-gray-500">Li e aceito os <a href="/termos" className="text-coral font-bold">Termos de Uso</a> e a <a href="/privacidade" className="text-coral font-bold">Política de Privacidade</a> da Avali.</p>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" checked={data.confirmTruth} onChange={e => setField('confirmTruth', e.target.checked)}
                      className="accent-coral mt-0.5 w-4 h-4" />
                    <p className="text-xs text-gray-500">Confirmo que as informações fornecidas são verdadeiras e baseadas na minha experiência real com esta escola. Estou ciente de que avaliações falsas ou ofensivas podem ser removidas.</p>
                  </label>
                </div>
              </div>
            )}
